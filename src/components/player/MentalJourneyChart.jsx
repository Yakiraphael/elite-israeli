...
function buildRadarData(assessment) {
  return RADAR_KEYS_17.map(k => ({
    subject: k.label,
    value: (assessment[k.key] || 0) * 20,
    category: k.category,
    fullMark: 100,
  }));
}
...
export default function MentalJourneyChart({ playerId }) {
  ...
  const { data: assessments = [], isLoading } = useQuery({
    ...
    queryFn: () => base44.entities.MentalAssessment.filter({ player_id: playerId }, '-assessment_date', 20),
    enabled: !!playerId,
  });
  ...
  const radarData = latestAssessment ? buildRadarData(latestAssessment) : [];
  ...
  if (!latestAssessment) {
    return (
      <div className="bg-[#0D1B2A] border border-white/10 rounded-lg p-8 text-center">
        ...
        <p className="text-white/40 text-sm">אין מבדקים מנטליים זמינים עדיין</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-white/40 text-xs">The Mental Resilience Journey · {assessments.length} מבדקים</p>
        </div>
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#D4AF37]/10 border border-[#D4AF37]/30">
          ...
          <span className="text-white/40 text-xs">/99</span>
        </div>
      </div>
      ...
      {timelineData.length > 1 && (
        <div className="bg-[#0D1B2A] border border-white/10 rounded-lg p-4">
          <p className="text-white/50 text-xs mb-3">ציר הזמן — ציון מנטלי משוקלל</p>
          <ResponsiveContainer width="100%" height={140}>
            <LineChart data={timelineData} onClick={(d) => d?.activePayload && setActivePoint(d.activePayload[0]?.payload)}>
              ...
              <YAxis domain={[0, 99]} tick={{ fontSize: 10, fill: '#ffffff40' }} />
              <Tooltip
                contentStyle={{ backgroundColor: '#1B263B', border: '1px solid #D4AF3730', borderRadius: 8, fontSize: 12 }}
                formatter={(v, _, p) => [`${v} (${p.payload.label})`, 'ציון מנטלי']}
              />
          ...
          {activePoint && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-3 bg-[#1B263B] border border-[#D4AF37]/20 rounded-lg p-3">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-white/40 text-xs">· {activePoint.team}</span>
                <span className="text-lg">{activePoint.label}</span>
              </div>
              <p className="text-white/60 text-xs leading-relaxed">{activePoint.comments || 'אין הערות'}</p>
            </motion.div>
          )}
        </div>
      )}
      ...
      {/* 17-axis Radar */}
      <div className="bg-[#0D1B2A] border border-white/10 rounded-lg p-4">
        <p className="text-white/50 text-xs mb-3">גרף רדאר — 17 מדדי חוסן (מבדק אחרון)</p>
        <ResponsiveContainer width="100%" height={280}>
          <RadarChart data={radarData}>
            <PolarGrid stroke="#ffffff15" />
            <PolarAngleAxis dataKey="subject" tick={{ fontSize: 8, fill: '#ffffff50' }} />
            <Radar name="מנטלי" dataKey="value" stroke="#D4AF37" fill="#D4AF37" fillOpacity={0.15} strokeWidth={2} />
            <Tooltip
              contentStyle={{ backgroundColor: '#1B263B', border: '1px solid #D4AF3730', borderRadius: 8, fontSize: 11 }}
              formatter={(v) => [`${Math.round(v / 20 * 10) / 10}/5`, '']}
            />
          </RadarChart>
        </ResponsiveContainer>
      </div>
      ...
      {latestAssessment.analyst_comments && (
        <div className="bg-[#0D1B2A] border border-[#D4AF37]/20 rounded-lg p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            {latestAssessment.assessment_date && <span className="text-white/30 text-xs">· {latestAssessment.assessment_date}</span>}
          </div>
          <p className="text-white/60 text-xs leading-relaxed">{latestAssessment.analyst_comments}</p>
          {latestAssessment.recommender_name && (
            <p className="text-white/30 text-[10px] mt-2">— {latestAssessment.recommender_name}</p>
          )}
        </div>
      )}
    </div>
