...
import { Building2, Plus, Trash2, Edit2, CheckCircle2, X, Loader2, ShieldCheck, Mail, Phone, MapPin, Award } from 'lucide-react';

const TIERS = [
  'Tier 1 — עלית (ליגת העל / בינלאומי)',
  'Tier 2 — מקצועי בינוני (לאומית / נוער ב׳)',
  'Tier 3 — חובבן / ליגות נמוכות',
];
...
const VERIFICATION_COLORS = {
  ...
  'נדחית': 'text-red-400 bg-red-400/10',
};

const emptyClub = () => ({
  club_name: '', club_tier: TIERS[2], subscription_plan: 'FREE',
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        ...
        <StatKpi label="ENTERPRISE" value={clubs.filter(c => c.subscription_plan === 'ENTERPRISE').length} icon={Award} color="#D4AF37" />
      </div>
          ...
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <Select label="סטטוס אימות" name="verification_status" value={form.verification_status} onChange={handleChange}
                options={['ממתין לאימות', 'מאומת', 'נדחית']} />
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  ...
                  <div className="text-white/30 text-[10px] mt-1">
                    {club.subscription_plan || 'FREE'} · {club.club_tier?.split(' — ')[0] || 'Tier 3'} · {club.contact_name || 'אין איש קשר'}
                  </div>
                </div>
