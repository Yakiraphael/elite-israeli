...
const NOTIFICATION_OPTIONS = [
  { key: 'medical_expiry', label: 'פקיעת תוקף אישור רפואי של הילד/ה' },
  { key: 'transfer_offer', label: 'הצעת העברה חדשה הממתינה לחתימתך' },
  { key: 'contract_status', label: 'עדכון סטטוס חוזה' },
];

export default function GuardianNotificationSettingsModal({ player, onClose }) {
  const [prefs, setPrefs] = useState(player.guardian_notification_preferences || {});

  const handleChange = async (key, checked) => {
    const updated = { ...prefs, [key]: checked };
    setPrefs(updated);
    await base44.entities.PlayerRegistration.update(player.id, { guardian_notification_preferences: updated });
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80" onClick={onClose}>
      <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
        ...
        <p className="text-white/40 text-xs mb-5">עבור {player.full_name}</p>
