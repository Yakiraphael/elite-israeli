    ...
    <section className="py-20 md:py-28 bg-white overflow-hidden relative">
      ...
      <div className="text-center mb-14 px-6">
        <motion.span initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}
          className="font-body text-xs tracking-[0.3em] text-gold font-bold uppercase">{s.gallery.badge}</motion.span>
        <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }}
          ...
          {s.gallery.title1} <span className="gold-gradient">{s.gallery.title2}</span>
        </motion.h2>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {PHOTOS.map((url, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }}
              whileHover={{ scale: 1.05 }} className="aspect-square rounded-lg overflow-hidden cursor-pointer shadow-md hover:shadow-xl transition-shadow"
              onClick={() => setSelected(url)}>
              <img src={url} alt={s.gallery.gridAlt} className="w-full h-full object-cover blur-sm" style={{ filter: 'blur(8px)' }} />
            </motion.div>
          ))}
        </div>
      </div>
      ...
      {selected && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-6" onClick={() => setSelected(null)}>
          <motion.img initial={{ scale: 0.85, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} src={selected}
            alt={s.gallery.lightboxAlt} className="max-w-3xl max-h-[85vh] w-full object-contain rounded-xl shadow-2xl" onClick={e => e.stopPropagation()} />
          <button onClick={() => setSelected(null)} className="absolute top-6 left-6 text-white/70 hover:text-white text-3xl leading-none">×</button>
        </motion.div>
      )}
    </section>
  );
