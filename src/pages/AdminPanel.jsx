...
function AdminDashboard({ onLogout }) {
  const [tab, setTab] = useState('mission_control');
    ...
    <div className="min-h-screen bg-[#0D1B2A]" dir="rtl">
      ...
      <div className="border-b border-white/10 bg-[#1B263B]">
        <div className="max-w-5xl mx-auto px-6 flex gap-0">
          {[{ id: 'mission_control', label: 'חדר בקרה עליון', icon: BarChart3 }, { id: 'clubs', label: 'מועדונים', icon: Building2 }, { id: 'club-docs', label: 'ארכיון מסמכים', icon: FolderOpen }, { id: 'events', label: 'אירועים', icon: Calendar }, { id: 'players', label: 'שחקנים', icon: Users }, { id: 'transfers', label: 'העברות', icon: Send }, { id: 'permissions', label: 'הרשאות', icon: ShieldCheck }, { id: 'audit', label: 'יומן ביקורת', icon: History }, { id: 'data_security', label: 'אבטחת מידע', icon: ShieldCheck }].map(t => (
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        {tab === 'mission_control' && <AdminMissionControl />}
        ...
        {tab === 'data_security' && <StorageSecurityPanel />}
      </div>
    </div>
  );
}
